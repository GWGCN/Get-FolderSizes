...
.PARAMETER GridView
    Use Gridview display instead of HTML report
.EXAMPLE
	.\Get-FolderSizes

	Run script and use defaults
.EXAMPLE
	.\Get-FolderSizes -Paths "c:\Windows" -ReportPath "c:\Scripts"
    
	Run the script and report on all folers in C:\Windows.  Save the
	HTML report in C:\Scripts
.EXAMPLE
	.\Get-FolderSizes -Paths "c:\Windows" -ReportPath "c:\Scripts" -Recurse    

	Run the script and report on all folers in C:\Windows.  Save the
	HTML report in C:\Scripts.  Report on all sub-folders.
.EXAMPLE
    .\Get-FolderSizes -Paths 'C:\temp', 'C:\Program Files' -sort Folder -descending

    Run the script and report on C:\temp and C:\Program Files sorting by folder name
.OUTPUTS
	FolderSizes.HTML in specified Report Path
.NOTES
    Mods by:        Colin Newlan
        1.8         Added Gridview switch to command line to provide alternative view to HTML report. Gridview allows
                    dynamic re-ordering and fitering of data.
                    Removed RawSize field from HTML report but re-ordered Sort and Select in Pipeline so HTML report will still be 
                    correctly ordered by RawSize when Sort order Size is selected.
                    RawSize field was retained in GridView for dynamic sorting using column headers.

        1.7         Added RawSize to report output and modified Sort by Size to numberic sort on RawSize, rather than alpha sort on Size string. 
                    Set default path to the users home drive\path, and the default report path to their Temp folder.
...
Param (
	...
	[string[]]$Paths="$Env:HOMEDRIVE$Env:HOMEPATH",
...
Begin {
    Function AddObject {
        ...
        Write-verbose "File system object: $FileObject"
...
Process {
    ...
    {   #Test if path exists
        ...
        #Display Primary Progress Bar
        Write-Progress -Activity "Target paths: $($Paths.count)" -Status "Processing path: $NumPath" -CurrentOperation "Current path: $Path" -PercentComplete ($NumPath/$Paths.count*100) -id 1
        ...
        $NumSubFolders = (Get-ChildItem @ParamSplat -Directory).Count
        Write-Verbose "Path: $Path    Subfolders:$NumSubFolders"
        $NumSubFolder=0
        ...
        {	
            ...
            #Display Secondary Progress Bar
            Write-Progress -Activity "Processing subfolders" -Status "Folder: $Folder" -PercentComplete ($NumSubfolder/$NumSubfolders*100) -ParentId 1 -id 2
...
"@
    ...
    if ($GridView) {
        #show gridview
        $Report | Sort $SortBy -Descending:$Descending| Select 'Folder Name',Owner,'Created On','Last Updated',Size, RawSize | Out-GridView -Wait
    } else {
